...
with dai.Device(pipeline) as device:
    ...
    ########### interface between ROS and OpenCV start HS #############
    bridge = CvBridge()
    ########### interface between ROS and OpenCV Ends HS #############
    ...
    ################ ROS shutdown per frames start HS #################
    while not rospy.is_shutdown():
        read_correctly, frame = get_frame()

        if not read_correctly:
            break
    ################ ROS shutdown per frames Ends HS #################
    ...
    #         break
        ...
        ########## publish camera frame to ROS starts HS ############
        ros_image = bridge.cv2_to_imgmsg(frame, encoding="rgb8")
        image_pub.publish(ros_image)
        ########## publish camera frame to ROS ends HS ############
